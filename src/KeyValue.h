...
void KeyValue::put_raw(daos_event_t *event, const char *key,
                       const char *value, size_t value_size) {
  DAOS_CHECK(daos_kv_put(object_handle_, DAOS_TX_NONE, 0, key, value_size,
                         value, event));
}

void KeyValue::change_value_raw() { throw std::runtime_error("Not implemented"); }

void KeyValue::get_raw(daos_event_t *event, const char *key) {
  size_t buffer_size = 128;
  char value[128];
  daos_kv_get(object_handle_, DAOS_TX_NONE, 0, key, &buffer_size, &value,
              event);
